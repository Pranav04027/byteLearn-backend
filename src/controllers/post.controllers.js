...
const createPost = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200 , "Created post successfully"))
})
...
const getUserPosts = asyncHandler(async (req, res) => {
  ...
  try {
    userposts = await Post.find({owner: userId})
  } catch (error) {
    throw new ApiError(400, "Some error occured while fetching posts");
  }
  ...
  res.status(200).json(new ApiResponse(200, userposts,"Fetched user posts successfully", userposts));
})
...
const updatePost = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, updatedpost, "Updated post successfully"));
})
